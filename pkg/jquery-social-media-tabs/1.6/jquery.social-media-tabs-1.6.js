	...
	$.extend(SocialTabsObject.prototype, {
		...
		create: function(el, options) {
			...
			$.each(this.o.widgets.split(','), function(i,v){
			  ...
			  $('.'+tabs,$c).append('<li class="'+cl+' btn btn-primary"><a rel="'+i+'" title="'+v+'"><i class="icon-'+self.o[v].icon+'" alt="" rel="'+v+'" ></i></a></li>');
		...
		addevents: function(a,t,s,l){
			...
			l.bind('click', function(e){
				var i = parseInt($(this).find('a').attr('rel'),10);
				if($(this).hasClass(ca)){
					if(m == 'slide'){
						self.close(a,l,s);
					}
					...
					$.each(a, function(i,item){
						if(i < o.limit){
							...
							switch(type)
							{
								...
								if(o.thumb == true){
									html.push('<a href="http://www.twitter.com/'+un+'" class="thumb"><img src="'+ua+'" alt="" /></a>');
								}
